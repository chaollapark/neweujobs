...
export default async function CategoryDetailPage({ params }: CategoryDetailPageProps) {
  ...
  const categoryJobs = await getJobsByCategory(slug, city)
      ...
      <div className="bg-eu-blue py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex text-sm text-gray-300 mb-6">
            ...
            <Link href={`/${city}/categories`} className="hover:text-white">Categories</Link>
            <span className="mx-2">/</span>
        ...
        {categoryJobs.length > 0 ? (
          <div className="space-y-4">
            {categoryJobs.map(job => (
              <JobCard key={job.id} job={job} featured={job.featured} citySlug={city} />
            ))}
          </div>
        ) : (
