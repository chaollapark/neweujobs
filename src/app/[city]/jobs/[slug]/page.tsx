...
export async function generateMetadata({ params }: JobDetailPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${job.title} at ${job.company.name}`,
      description: job.description.replace(/<[^>]*>/g, '').slice(0, 160),
      url: `https://eujobs.co/${city}/jobs/${job.slug}`,
      siteName: 'EU Jobs',
      type: 'website',
    },
  }
...
export default async function JobDetailPage({ params }: JobDetailPageProps) {
  ...
  const activeJobs = isRetired ? await getLatestJobs(3, city) : []
  ...
  const shareUrl = `https://eujobs.co/${city}/jobs/${job.slug}`
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex text-sm text-gray-500 dark:text-gray-400">
            <Link href={`/${city}`} className="hover:text-eu-blue">Home</Link>
            <span className="mx-2">/</span>
            <Link href={`/${city}/jobs`} className="hover:text-eu-blue">Jobs</Link>
            <span className="mx-2">/</span>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-8 mb-6">
              <div className="flex items-start gap-6">
                ...
                <div className="flex-1">
                  ...
                  <Link
                    href={`/${city}/companies/${job.company.slug}`}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 sticky top-24">
              {isRetired ? (
                <div className="mb-4">
                  <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-4 text-center">
                    ...
                    <Link href={`/${city}/jobs`} className="btn-primary w-full mt-3 text-sm py-3 block text-center">
                      Browse Active Jobs
                    </Link>
                  </div>
                </div>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    <Link href={`/${city}/categories/${job.category.slug}`} className="text-eu-blue hover:underline">
                      {job.category.name}
                    </Link>
                  </p>
                </div>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 mt-6 pt-6">
                ...
                <div className="flex gap-2">
                  ...
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`${job.title} at ${job.company.name}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
        ...
        {isRetired && activeJobs.length > 0 && (
          <section className="mt-10">
            ...
            <div className="mt-6 text-center">
              <Link href={`/${city}/jobs`} className="text-eu-blue dark:text-blue-400 font-medium hover:underline">
