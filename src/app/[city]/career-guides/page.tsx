...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  return {
    title: 'Organization Career Guides',
    ...
    openGraph: {
      title: 'Organization Career Guides - EU Jobs',
      description: 'Browse career guides for over 12,000 EU lobbying organizations.',
      url: `https://eujobs.co/${city}/career-guides`,
      siteName: 'EU Jobs',
      type: 'website',
    },
  }
...
export default async function CareerGuidesListingPage({ params, searchParams }: PageProps) {
  const { city } = await params
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {guides.map((guide: any) => (
            <Link
              key={guide.slug}
              href={`/${city}/career-guides/${guide.slug}`}
        ...
        {totalPages > 1 && (
          <nav className="mt-12 flex items-center justify-center gap-2">
            {page > 1 && (
              <Link
                href={`/${city}/career-guides?page=${page - 1}`}
