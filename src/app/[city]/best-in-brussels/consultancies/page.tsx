...
export default async function ConsultanciesPage({ params, searchParams }: PageProps) {
  const { city } = await params;
  const resolvedSearchParams = await searchParams;
  const currentPage = Math.max(1, parseInt(resolvedSearchParams.page || '1', 10));
  const specialism = resolvedSearchParams.specialism || '';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <BibSpecialismChips
          ...
          basePath={`/${city}/best-in-brussels/consultancies`}
        />
        ...
        {items.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item: any) => (
              <BibCard
                key={item._id}
                href={`/${city}/best-in-brussels/consultancies/${item.slug}`}
                name={item.name}
