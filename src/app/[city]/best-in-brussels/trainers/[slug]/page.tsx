...
import { getBibTrainerBySlug } from '@/lib/bibData';

export const dynamicParams = true;
export const revalidate = 86400;

interface PageProps { params: Promise<{ city: string; slug: string }>; }

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city, slug } = await params;
  const item = await getBibTrainerBySlug(slug);
  if (!item) return { title: 'Trainer Not Found' };
  return { title: `${item.name} - EU Public Affairs Trainer`, description: item.description?.substring(0, 160) };
}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <section className="bg-gradient-to-br from-eu-blue to-eu-dark text-white py-12">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href={`/${city}/best-in-brussels/trainers`} className="inline-flex items-center text-gray-300 hover:text-white mb-6 transition-colors">&larr; Back to trainers</Link>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {item.programs?.length > 0 && (
              <section className="bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-gray-900/30 border border-gray-200 dark:border-gray-600 p-6">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-3">Training Programs</h2>
                <ul className="space-y-2">
                  {item.programs.map((p: string, i: number) => (
                    <li key={i} className="text-gray-600 dark:text-gray-300 flex items-start gap-2"><span className="text-eu-blue mt-1">&#8226;</span><span>{p}</span></li>
                  ))}
                </ul>
              </section>
            )}
