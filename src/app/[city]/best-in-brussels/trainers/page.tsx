      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {items.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item: any) => (
              <BibCard key={item._id} href={`/${city}/best-in-brussels/trainers/${item.slug}`} name={item.name} description={item.description} logoUrl={item.logoUrl} tags={item.programs} />
            ))}
          </div>
        )}
      </div>
